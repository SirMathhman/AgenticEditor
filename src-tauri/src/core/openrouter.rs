...
type ToolExecutor = Box<dyn Fn(&str) -> Result<String, AppError> + Send + Sync>;
...
fn tool_fn(
    root: &std::sync::Arc<std::path::Path>,
    f: impl Fn(&std::path::Path, &str) -> Result<String, AppError> + Send + Sync + 'static,
) -> ToolExecutor {
    let root = root.clone();
...
fn tools(root: &std::path::Path) -> Vec<Tool> {
    let root: std::sync::Arc<std::path::Path> = std::sync::Arc::from(root.to_path_buf());
    vec![
        Tool {
            name: "get_local_time",
...
fn execute_tool(root: &std::path::Path, name: &str, arguments: &str) -> Result<String, AppError> {
...
pub fn chat_with_tools(
    ...
    for _ in 0..MAX_TOOL_ROUNDS {
        ...
        for call in &reply.tool_calls {
            let result =
                execute_tool(root, &call.name, &call.arguments).unwrap_or_else(|e| e.to_string());
